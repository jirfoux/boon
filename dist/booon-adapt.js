...
(function () {
    class Adapt {
        constructor(opts) {
            ...
            ["methods", "watch", "options", "validate"].forEach((name) =>
                Object.freeze(opts[name] || {})
            );
            ...
            const load = () => {
                if (opts.el instanceof Element) {
                    ...
                    Array.isArray(opts.el) &&
                    opts.el.every((e) => e instanceof Element)
                ) {
                    that._el = Array.from(opts.el);
                ...
                if (dir == "attr") {
                    if (typeof calculatedValue !== "object") {
                        ...
                        if (Array.isArray(calculatedValue)) {
                            calculatedValue.forEach((e) => {
                                if (e) node.classList.add(e);
                            });
                        } else {
                    ...
                    if (uf.initStyle) {
                        node.setAttribute(
                            "style",
                            uf.initStyle + node.getAttribute("style")
                        );
                    }
                }
            ...
            .flatMap((node) => {
                ...
                if (node.nodeType == 1) {
                    ...
                    attrNames.forEach((name) => {
                        ...
                        if (
                            name.startsWith("b-attr:") ||
                            name.startsWith(":")
                        ) {
                            result.dir = "attr";
                            result.attribute = name.slice(
                                name.indexOf(":") + 1
                            );
                    ...
                    for (let i = 0; i < endInds.length; i++) {
                        const key = node.textContent.substring(
                            startInds[i],
                            endInds[i] + end.length
                        );
                        const val = key.slice(
                            start.length,
    ...
    const getFunction = (adapt, expression, isEvent) => {
        ...
        const isFunc =
            Boolean((adapt._options.methods || {})[expression]) ||
            (expression.endsWith(")") &&
        ...
        if (isFunc) {
            if (!expression.endsWith(")")) {
                ...
                funcBody +=
                    funcName +
                    ".apply(this,[" +
                    expression.slice(paraIndex + 1, expression.length - 1) +
                    "])";
            }
        } else {
            funcBody += expression;
