...
window.booon = (function () {
    ...
    class Booon {
        ...
        addClass(clazz) {
            return this.each((element) =>
                element.classList.add(...toArray(clazz))
            );
        }

        removeClass(clazz) {
            return this.each((element) =>
                element.classList.remove(...toArray(clazz))
            );
        }

        toggleClass(clazz) {
            return this.each((element) =>
                toArray(clazz).forEach((c) => element.classList.toggle(c))
            );
        }
        ...
        parent() {
            return booon(
                distinct(this.es.map((element) => element.parentElement))
            );
        }
        ...
        children() {
            return booon(
                distinct(
                    this.es.flatMap((element) => Array.from(element.children))
                )
            );
        }
        ...
        siblings(inclusive) {
            if (inclusive) {
                ...
                return booon(
                    distinct(this.es.flatMap((element) => s(element)))
                );
            }
        }
        ...
        merge(value) {
            ...
            this.each((element) => {
                if (!newElements.includes(element)) newElements.push(element);
            });
    ...
    const booon = (value, ...args) => {
        ...
        if (value instanceof Booon) {
            ...
            return booon(
                (booon(args[0])[0] || document).querySelectorAll(value)
            );
        } else {
            return new Booon([]);
        }
